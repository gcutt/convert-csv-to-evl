...
for (i in seq_len(nrow(neutral_runs))) {
  ...
  # Look at slope entering the neutral zone
  ...
  df$turning_point[s + idx - 1] <- TRUE
}

# --- Propagate direction between turning points ---
tp_idx <- which(df$turning_point)
...
#     ) +
#     scale_y_reverse() +
#     coord_cartesian(ylim = c(20, -5)) +   # zoom without filtering
#     theme_minimal()
#   }
  ...
  ggplot(df, aes(x = g_utc_time, y = .data[[depth_col]])) +
    ...
    scale_color_manual(
      values = c(
        ...
        `0`  = "grey70",   # neutral
        `1`  = "cyan",     # descent
        `NA` = "grey50"    # optional: color for NA if present
      ),
      breaks = c(-1, 0, 1),
  ...
  ggplot(df, aes(x = g_utc_time, y = .data[[depth_col]])) +
    ...
    scale_color_manual(
      values = c(
        ...
        `0`  = "purple",   # neutral
        `1`  = "green3",     # descent
        `NA` = "red"    # optional: color for NA if present
      ),
      breaks = c(-1, 0, 1),
      labels = c("Ascent", "Neutral", "Descent"),
      name   = "Direction"
