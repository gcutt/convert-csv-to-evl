...
## NEW 
# # --- Smooth depth for turning point detection ---
# df$z_med <- zoo::rollmedian(df[[depth_col]], k = win_pts, fill = NA, align = "center")
# 
# # --- Find turning points (local maxima + minima) ---
# find_turning_points <- function(z) {
#   # local maxima
#   peaks   <- which(diff(sign(diff(z))) == -2) + 1
#   # local minima
#   valleys <- which(diff(sign(diff(z))) ==  2) + 1
#   list(peaks = peaks, valleys = valleys)
# }
# 
# tp <- find_turning_points(df$z_med)
# 
# # Add to df
# df$turning_point <- FALSE
# df$turning_point[tp$peaks]   <- TRUE
# df$turning_point[tp$valleys] <- TRUE

# Smaller smoothing window for turning points
win_pts_tp <- 11

df$z_med_tp <- zoo::rollmedian(df[[depth_col]], k = win_pts_tp, fill = NA, align = "center")
...
find_turning_points <- function(z) {
  ...
  dz  <- diff(z2)
  ddz <- diff(sign(dz))
  peaks   <- good[ which(ddz == -2) + 1 ]
  valleys <- good[ which(ddz ==  2) + 1 ]
  list(peaks = peaks, valleys = valleys)
}

tp <- find_turning_points(df$z_med_tp)
...
if( OPTIONS_PLOT ){
  ggplot(df, aes(x = g_utc_time, y = .data[[depth_col]])) +
    ...
    labs(
      title = "Glider depth vs time with turning points",
      x = "Time (UTC)",
      y = "Depth (m)"
    ) +
    theme_minimal()
