...
#     try:
#         f = open(evd_path, "rb")
#         print(f"DEBUG: file opened OK: {evd_path}")
#     except Exception as e:
#         print(f"DEBUG: file open FAILED: {e}")
#         return
#
...
#
#     # --- First bytes ---
#     print("DEBUG: first 200 bytes:", data[:200])
#
#     # --- First <Packet> raw bytes ---
#     first_packet = data.find(b"<Packet")
#     print("DEBUG: first <Packet> index:", first_packet)
#     if first_packet != -1:
#         print("DEBUG: raw first <Packet> bytes:", data[first_packet:first_packet + 120])
#     else:
#         print("DEBUG: NO <Packet found in entire file!")
#
#     # --- Regex sanity check ---
#     print("\nDEBUG: Testing PACKET_OPEN_RE against first 500 bytes...")
#     test_match = PACKET_OPEN_RE.search(data[:500])
#     print("DEBUG: PACKET_OPEN_RE test match:", test_match)
#
#     # --- Find ALL matches up front ---
#     print("\nDEBUG: Scanning entire file for PACKET_OPEN_RE matches...")
#     all_matches = list(PACKET_OPEN_RE.finditer(data))
#     print(f"DEBUG: PACKET_OPEN_RE found {len(all_matches)} matches total")
#
#     for i, m in enumerate(all_matches[:5]):
#         print(f"DEBUG: match {i}: starts at {m.start()}, bytes: {m.group(0)[:80]}")
#
#     if len(all_matches) == 0:
#         print("DEBUG: NO PACKET TAGS FOUND — iterator will yield nothing")
#         return
#
#     # --- Main scanning loop ---
#     print("\nDEBUG: ENTERING MAIN PACKET LOOP")
...
#         m_next = PACKET_OPEN_RE.search(data, m_open.end())
#         print("DEBUG: next packet match:", m_next)
#
#         if m_next:
#             end = m_next.start()
#         else:
...
#         try:
#             header_end = block.find(b'>') + 1
#             header = block[:header_end]
#             print("DEBUG: header bytes:", header[:120])
#
#             elem = ET.fromstring(header.decode("utf-8", errors="ignore"))
#             print("DEBUG: SUCCESS: parsed XML element:", elem.tag)
#
#             yield elem
#
#         except Exception as e:
...
#
#     first_packet = data.find(b"<Packet")
#     print("DEBUG: raw first <Packet> bytes:", data[first_packet:first_packet + 80])
#
#     pos = 0
#     n = len(data)
#
#     print(f"DEBUG: iterator reading: {evd_path}, len(data) {len(data)}")
#
#     while True:
#         m_open = PACKET_OPEN_RE.search(data, pos)
#         if not m_open:
...
def main():
    ...
    # Write to json
    json_path = os.path.splitext(evd_path)[0] + "_acmeta_v01-6.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(acmeta, f, indent=2)

    # write to csv
    csv_path = evd_path.replace(".evd", "_acmeta_v01-6.csv")
